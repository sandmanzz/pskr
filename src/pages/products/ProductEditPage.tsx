...
import { ArrowLeft, Bold, Heading2, Italic, List, ListOrdered, Plus, Quote, X } from "lucide-react"
...
export function ProductEditPage() {
  ...
  const product = isNew ? null : (products.find((p) => p.id === Number(id)) ?? null)
  ...
  const pageSubtitle = isNew ? "Fill in the details for the new product" : `Update "${product?.name}" Products Information`
  ...
  function handleSave() {
    ...
    if (isNew) {
      ...
      updateProduct(Number(id), { name: name.trim(), price: Number(price) || 0, shortSummary, productSummary, category })
    }
    setSaved(true)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        ...
        <Button onClick={() => navigate("/products")} className="bg-white/10 hover:bg-white/20 text-white border border-white/20 gap-1.5">
      ...
      <div className="px-8 py-3 border-b border-gray-200 bg-white">
        <p className="text-sm text-gray-500">
          ...
          <span className="text-gray-400 cursor-pointer hover:text-gray-600" onClick={() => navigate("/products")}>Products</span>
          <span className="mx-1.5 text-gray-300">›</span>
          {!isNew && <><span className="text-gray-400">{id}</span><span className="mx-1.5 text-gray-300">›</span></>}
          <span>edit</span>
        </p>
      </div>
      ...
      <div className="px-8 py-6 max-w-5xl">
        ...
        {saved && (
          <div className="mb-4 rounded-md bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">
            {isNew ? "Produk berhasil dibuat!" : "Produk berhasil diupdate!"} Mengalihkan...
          </div>
        )}
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-2 gap-5 mb-5">
              <div className="space-y-1.5">
                ...
                <Input id="name" value={name} onChange={(e) => setName(e.target.value)} className="border-gray-200" />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="price" className="text-sm font-medium text-gray-700">
                  Product Price (¥) <span className="text-red-500">*</span>
                </Label>
                <Input id="price" value={price} onChange={(e) => setPrice(e.target.value)} className="border-gray-200" type="number" />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <RichTextEditor id="product-summary" value={productSummary} onChange={setProductSummary} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm border-gray-200 mb-8">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-gray-900">
                Product Images{" "}
                <span className="text-sm font-normal text-gray-400">{(product?.images ?? []).length}/8</span>
              </h3>
            </div>
        ...
        <div className="flex items-center gap-3">
          <Button className="bg-gray-900 hover:bg-gray-800 text-white px-8" onClick={handleSave}>
...
function RichTextEditor({ id, value, onChange }: { id: string; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="overflow-hidden rounded-md border border-gray-200 bg-white focus-within:ring-2 focus-within:ring-ring">
      <div className="flex flex-wrap items-center gap-1 border-b border-gray-200 bg-gray-50 px-2 py-2">
        <EditorButton label="Heading" onClick={() => runCommand("formatBlock", "h2")}><Heading2 className="h-4 w-4" /></EditorButton>
        <EditorButton label="Bold" onClick={() => runCommand("bold")}><Bold className="h-4 w-4" /></EditorButton>
        <EditorButton label="Italic" onClick={() => runCommand("italic")}><Italic className="h-4 w-4" /></EditorButton>
        <div className="mx-1 h-6 w-px bg-gray-200" />
        <EditorButton label="Bulleted list" onClick={() => runCommand("insertUnorderedList")}><List className="h-4 w-4" /></EditorButton>
        <EditorButton label="Numbered list" onClick={() => runCommand("insertOrderedList")}><ListOrdered className="h-4 w-4" /></EditorButton>
        <EditorButton label="Quote" onClick={() => runCommand("formatBlock", "blockquote")}><Quote className="h-4 w-4" /></EditorButton>
      </div>
      <div
        id={id}
        ref={editorRef}
...
function EditorButton({ label, children, onClick }: { label: string; children: ReactNode; onClick: () => void }) {
  return (
    <button type="button" aria-label={label} title={label} onClick={onClick} className="inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-600 hover:bg-white hover:text-gray-900">
      {children}
    </button>
  )
}
