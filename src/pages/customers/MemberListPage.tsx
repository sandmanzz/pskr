...
export function MemberListPage() {
  ...
  const { members, deleteMember } = useAppStore()
  const [search, setSearch] = useState("")

  const filteredMembers = useMemo(() => {
    const q = search.trim().toLowerCase()
    if (!q) return members
    return members.filter((m) => [m.email, m.status].some((v) => v.toLowerCase().includes(q)))
  }, [search, members])
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      >
        ...
        <Button className="bg-white/10 hover:bg-white/20 text-white border border-white/25 gap-1.5" onClick={() => navigate("/customers/member/new")}>
      ...
      <main className="px-10 py-5">
        ...
        <div className="grid grid-cols-3 gap-5 mb-7">
          ...
          <StatCard label="Inactive Members" value={stats.inactive} />
        </div>
        ...
        <div className="rounded-b border border-t-0 border-slate-200 bg-white">
          {filteredMembers.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <Button className="bg-slate-950 hover:bg-slate-800 text-white gap-1.5" onClick={() => navigate("/customers/member/new")}>
                  ...
                  {filteredMembers.map((m) => (
                    <tr key={m.id} className="border-b border-slate-200 last:border-b-0 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${m.status === "active" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"}`}>
                          {m.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-500">{m.createdAt}</td>
                      <td className="px-4 py-3">
                        <button onClick={() => handleDelete(m.id)} className="p-1.5 rounded hover:bg-red-50 text-slate-400 hover:text-red-500">
